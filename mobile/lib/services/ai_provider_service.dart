...
class AiProviderService {
  ...
  Future<List<Map<String, String>>> extractEntities(Note note) async {
    ...
    if (s.provider == AiProvider.rules) return out;
    ...
    text = text.replaceFirst(RegExp(r'\s*~~~    final s = await settings();
    if (s.provider == AiProvider.rules) {
  ...
  String _errorMessage(String body) {
    ...
    text = text.replaceFirst(
      RegExp(r'^\x60\x60\x60(?:json)?\s*', caseSensitive: false),
      '',
    );
    text = text.replaceFirst(RegExp(r'\s*\x60\x60\x60    final s = await settings();
    if (s.provider == AiProvider.rules) {
      throw StateError('El modo Reglas no usa un modelo de IA.');
    }
  ...
  String _errorMessage(String body) {
    ...
    if (decoded is List) {
      for (final item in decoded) {
        ...
        var type = (item['type'] ?? 'persona').toString().trim().toLowerCase();
        ...
        if (!duplicate) out.add({'name': name, 'type': type});
      }
    }

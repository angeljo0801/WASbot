...
class OcrPurchaseService {
  ...
  Future<Map<String, String>> _resolveCustomerFromOcr(String text) async {
    ...
    final normalizedCandidate = store.normalizeName(candidate);

    List<Map<String, dynamic>> matching(String value) {
      final n = store.normalizeName(value);
    ...
    if (normalizedCandidate.isNotEmpty) {
      final matches = matching(candidate);
      if (matches.length == 1) {
        return {
